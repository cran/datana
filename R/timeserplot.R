...
#' @param col.lines A string specifying the single color to be used for the lines of the timeseries
#' 
...
#' 
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
timeserplot <- function (data = data, y = y, x = x, obs.unit = obs.unit, factor1 = NA, 
                         factor2 = NA, only.lines = FALSE, ylab = NA, xlab = NA, linetype.lab = NA, 
                         factor2.line = TRUE, factor2.col = FALSE, max.y.all = NA, 
                         levels.i.want = FALSE, col.lev.i.want = FALSE, col.lines = FALSE) {
  db <- data
  if (prod(is.na(factor1))) {
    has.factor1 <- 0
  }  else {
    has.factor1 <- 1
  }
  if (prod(is.na(factor2))) {
    has.factor2 <- 0
  }  else {
    has.factor2 <- 1
  }
  if (has.factor1 == 0 & has.factor2 == 0) {
    df.h <- data.frame(assign(obs.unit, as.factor(db[, obs.unit])), 
                       assign(x, db[, x]), 
                       assign(y, db[, y]))
    names(df.h) <- c(obs.unit, x, y)
  }  else {
    nada = 111
  }
  if (has.factor1 == 1 & has.factor2 == 0) {
    df.h <- data.frame(assign(obs.unit, db[, obs.unit]), 
                       assign(factor1, db[, factor1]), 
                       assign(x, db[, x]), 
                       assign(y, db[, y]))
    names(df.h) <- c(obs.unit, factor1, x, y)
  }  else {
    nada = 111
  }
  if (has.factor1 == 1 & has.factor2 == 1) {
    df.h <- data.frame(assign(obs.unit, db[, obs.unit]), 
                       assign(factor1, db[, factor1]), 
                       assign(factor2, db[, factor2]), 
                       assign(x, db[, x]), 
                       assign(y, db[, y]))
    names(df.h) <- c(obs.unit, factor1, factor2, x, y)
  }  else {
    nada = 111
  }
  if (prod(is.na(xlab))) {
  ...
  if (prod(is.na(linetype.lab)) & col.lines==FALSE) {
    linetype.lab.h <- factor2
  }  else {
    linetype.lab.h <- linetype.lab
  }
  if (prod(is.na(linetype.lab)) & col.lines == TRUE) {
    linetype.lab.h <- obs.unit
  }  
  if (col.lines == FALSE) {
    col.lines.h <- "gray"} else {
    col.lines.h <- col.lines
  }  

  
  if (has.factor1 == 0 & has.factor2 == 0 & only.lines == FALSE) {
    ggplot2::ggplot(df.h, ggplot2::aes(x = df.h[, x], 
                                       y = df.h[, y], 
                                       group = df.h[, obs.unit]))
    p <- ggplot2::ggplot(df.h, ggplot2::aes(x = df.h[, x], 
                                            ...
                                            group = df.h[, obs.unit])) + 
      ggplot2::geom_line() + 
      ggplot2::geom_point() + ggplot2::xlab(xlab.h) + ggplot2::ylab(ylab.h)
  ...
  if (has.factor1 == 0 & has.factor2 == 0 & only.lines == FALSE & col.lines==TRUE) {
    ggplot2::ggplot(df.h, ggplot2::aes(x = df.h[, x], 
                                       y = df.h[, y], 
                                       group = df.h[, obs.unit]))
    p <- ggplot2::ggplot(df.h, ggplot2::aes(x = df.h[, x], 
                                            y = df.h[, y], 
                                            group = df.h[, obs.unit])) + 
      ggplot2::geom_line(ggplot2::aes(col = df.h[, obs.unit])) + 
      ggplot2::geom_point(ggplot2::aes(col = df.h[, obs.unit])) + 
      ggplot2::xlab(xlab.h) + 
      ggplot2::ylab(ylab.h) +
      ggplot2::labs(col = linetype.lab.h)
    p <- p + ggplot2::theme(panel.background = ggplot2::element_blank(), 
                            panel.border = ggplot2::element_blank(), panel.grid.major = ggplot2::element_blank(), 
                            panel.grid.minor = ggplot2::element_blank(), axis.line = ggplot2::element_line(colour = "blue"))
    print(p)
  }  else {
    nada = 0
  } ####acaaaa
  if (has.factor1 == 0 & has.factor2 == 0 & only.lines == TRUE) {
    ...
    p <- ggplot2::ggplot(df.h, ggplot2::aes(x = df.h[, x], 
                                            y = df.h[, y], 
                                            group = df.h[, obs.unit])) + 
      ggplot2::geom_line(color = col.lines.h) +
  ...
  if (has.factor1 == 0 & has.factor2 == 0 & only.lines == TRUE & col.lines==TRUE) {
    ggplot2::ggplot(df.h, ggplot2::aes(x = df.h[, x], 
                                       y = df.h[, y], 
                                       group = df.h[, obs.unit]))
    p <- ggplot2::ggplot(df.h, ggplot2::aes(x = df.h[, x], 
  ...
  if (has.factor1 == 1 & has.factor2 == 0 & only.lines == FALSE) {
    p <- ggplot2::qplot(y = df.h[, y], x = df.h[, x], group = df.h[, 
                                                                   obs.unit], facets = ~df.h[, factor1], data = df.h, 
                        geom = c("point", "line"), ylab = ylab.h, xlab = xlab.h)
    p <- p + ggplot2::theme(panel.background = ggplot2::element_blank(), 
                            panel.border = ggplot2::element_blank(), panel.grid.major = ggplot2::element_blank(), 
                            panel.grid.minor = ggplot2::element_blank(), axis.line = ggplot2::element_line(colour = "black"))
    print(p)
  ...
  if (has.factor1 == 1 & has.factor2 == 0 & only.lines == TRUE) {
    p <- ggplot2::qplot(y = df.h[, y], x = df.h[, x], group = df.h[, 
                                                                   obs.unit], facets = ~df.h[, factor1], data = df.h, 
                        geom = c("line"), ylab = ylab.h, xlab = xlab.h)
    p <- p + ggplot2::theme(panel.background = ggplot2::element_blank(), 
                            panel.border = ggplot2::element_blank(), panel.grid.major = ggplot2::element_blank(), 
                            panel.grid.minor = ggplot2::element_blank(), axis.line = ggplot2::element_line(colour = "black"))
    print(p)
  ...
  if (has.factor1 == 1 & has.factor2 == 1 & factor2.col == 
      TRUE) {
    factor2.line = FALSE
  }  else {
    nada = 999
  }
  if (has.factor1 == 1 & has.factor2 == 1 & factor2.line == 
      TRUE & only.lines == FALSE) {
    p <- ggplot2::ggplot(df.h, ggplot2::aes(x = df.h[, x], 
                                            y = df.h[, y], 
                                            group = df.h[, obs.unit])) + 
  ...
  if (has.factor1 == 1 & has.factor2 == 1 & factor2.line == 
      TRUE & only.lines == TRUE) {
    p <- ggplot2::ggplot(df.h, ggplot2::aes(x = df.h[, x], 
                                            y = df.h[, y], group = df.h[, obs.unit])) + ggplot2::geom_line(ggplot2::aes(linetype = df.h[, 
                                                                                                                                        factor2])) + ggplot2::facet_wrap(df.h[, factor1]) + 
      ggplot2::xlab(xlab.h) + ggplot2::ylab(ylab.h) + ggplot2::labs(linetype = linetype.lab.h)
    p <- p + ggplot2::theme(panel.background = ggplot2::element_blank(), 
                            panel.border = ggplot2::element_blank(), panel.grid.major = ggplot2::element_blank(), 
                            panel.grid.minor = ggplot2::element_blank(), axis.line = ggplot2::element_line(colour = "black"))
    print(p)
  ...
  if (has.factor1 == 1 & has.factor2 == 1 & factor2.col == 
      TRUE & only.lines == FALSE) {
    p <- ggplot2::ggplot(df.h, ggplot2::aes(x = df.h[, x], 
                                            y = df.h[, y], group = df.h[, obs.unit])) + ggplot2::geom_line(ggplot2::aes(col = df.h[, 
                                                                                                                                   factor2])) + ggplot2::facet_wrap(df.h[, factor1]) + 
      ggplot2::geom_point() + ggplot2::xlab(xlab.h) + ggplot2::ylab(ylab.h) + 
      ggplot2::labs(col = linetype.lab.h)
    p <- p + ggplot2::theme(panel.background = ggplot2::element_blank(), 
  ...
  if (has.factor1 == 1 & has.factor2 == 1 & factor2.col == 
    ...
    p <- ggplot2::ggplot(df.h, ggplot2::aes(x = df.h[, x], 
      ...
      ggplot2::facet_wrap(df.h[, factor1]) + 
      ggplot2::xlab(xlab.h) + ggplot2::ylab(ylab.h) + ggplot2::labs(col = linetype.lab.h)
    p <- p + ggplot2::theme(panel.background = ggplot2::element_blank(), 
                            panel.border = ggplot2::element_blank(), panel.grid.major = ggplot2::element_blank(), 
                            panel.grid.minor = ggplot2::element_blank(), axis.line = ggplot2::element_line(colour = "black"))
    print(p)
